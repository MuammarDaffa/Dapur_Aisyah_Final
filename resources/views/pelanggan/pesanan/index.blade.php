...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="d-flex d-flex-wrap g-3 mb-6">
        <a href="{{ route('pelanggan.pesanan') }}" class="px-4 py-2 rounded-pill fs-6 fw-medium {{ !request('status') ? 'bg-primary text-white text-white' : 'bg-light text-secondary hover:bg-light' }}">Semua</a>
        @foreach(['diproses' => 'Diproses', 'dikirim' => 'Dikirim', 'selesai' => 'Selesai', 'dibatalkan' => 'Dibatalkan'] as $key => $label)
            <a href="{{ route('pelanggan.pesanan', ['status' => $key]) }}" class="px-4 py-2 rounded-pill fs-6 fw-medium {{ request('status') == $key ? 'bg-primary text-white text-white' : 'bg-light text-secondary hover:bg-light' }}">{{ $label }}</a>
        @endforeach
    </div>

    <div class="d-flex flex-column gap-3">
        @forelse($pesanan as $pesanan)
            <a href="{{ route('pelanggan.pesanan.show', $pesanan) }}" class="d-block bg-white rounded p-5 shadow-sm border border border-secondary hover:shadow-md transition-shadow">
