...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="rounded-2xl p-8 mb-8 text-white">
        ...
        <a href="{{ route('customer.products') }}" class="d-inline-d-flex align-items-center mt-4 px-6 py-2.5 bg-white text-primary fw-bold rounded-pill hover:shadow">
        ...
        <div class="card shadow-sm mb-4 p-4">
            <div class="d-flex align-items-center justify-content-between">
                <div>
                    ...
                    <p class="fs-3 fw-bold text-secondary">{{ auth()->user()->orders()->count() }}</p>
                </div>
        ...
        <div class="card shadow-sm mb-4 p-4">
            <div class="d-flex align-items-center justify-content-between">
                <div>
                    ...
                    <p class="fs-3 fw-bold text-secondary">{{ auth()->user()->reviews()->count() }}</p>
                </div>
                <div style="width: 48px; height: 48px;" class="rounded bg-primary text-white text-primary d-flex align-items-center justify-content-center flex-shrink-0">
    ...
    <div class="bg-white rounded shadow-sm border border border-secondary">
        <div class="p-6 border-b border border-secondary d-flex justify-content-between align-items-center">
            ...
            <a href="{{ route('customer.orders') }}" class="d-inline-d-flex align-items-center fs-6 text-primary hover:text-primary fw-medium">
        ...
        <div class="divide-y divide-gray-100">
            @forelse($recentOrders as $order)
                <a href="{{ route('customer.orders.show', $order) }}" class="d-flex align-items-center justify-content-between p-4 hover:bg-primary text-white/50">
                    <div class="d-flex align-items-center space-x-4">
                        ...
                        <div>
                            <p class="fw-medium text-secondary">{{ $order->order_number }}</p>
                            <p class="small text-secondary">{{ $order->cateringService->name ?? 'Katering' }} · {{ $order->created_at->format('d M Y') }}</p>
                        </div>
                    </div>
                    <div class="text-end">
                        <p class="fw-bold text-secondary">Rp {{ number_format($order->total, 0, ',', '.') }}</p>
                        <span class="d-inline-block small px-2 py-0.5 rounded-pill fw-medium {{ match($order->status) { 'processing' => 'bg-info text-white text-info', 'on_delivery' => 'bg-purple-100 text-purple-700', 'completed' => 'bg-success text-white text-success', 'cancelled' => 'bg-danger text-white text-danger', default => 'bg-light text-secondary' } }}">
                            {{ $order->status_label }}
                        </span>
                    </div>
                </a>
            @empty
                <div class="p-8 text-center text-secondary">
                    ...
                    <p>Belum ada pesanan. <a href="{{ route('customer.products') }}" class="text-primary fw-medium hover:underline">Pesan sekarang</a></p>
                </div>
            @endforelse
        </div>
    </div>
