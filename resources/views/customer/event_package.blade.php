...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    {{-- Header --}}
    <div class="mb-6">
        <a href="{{ route('customer.event.service', $service->id) }}" class="d-inline-d-flex align-items-center fs-6 fw-medium text-primary hover:text-primary">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border border-secondary overflow-hidden mb-8">
        ...
        @if($package->image)
            <div style="height: 256px;" class="w-100 sm:h-80 bg-light position-relative overflow-hidden">
                <img src="{{ Storage::url($package->image) }}" alt="{{ $package->name }}" class="w-100 h-100 object-cover">
            </div>
        @endif
        ...
        <div class="p-6 sm:p-10">
            ...
            <div>
                <div class="d-flex d-flex-column sm:d-flex-row sm:align-items-center justify-content-between g-3">
                    <div>
                        ...
                        <h1 class="fs-3 sm:fs-2 fw-bold text-secondary">{{ $package->name }}</h1>
                        <p class="fs-6 fw-medium text-secondary mt-1">Layanan: {{ $service->name }}</p>
                    </div>
                    <div class="text-start sm:text-end">
                        <p class="small text-secondary">Harga Paket</p>
                        <p class="fs-3 sm:fs-2 fw-bold text-primary">{{ $package->formatted_price }}</p>
                        <p class="small fw-bold text-secondary mt-1">{{ $package->total_portions }} Porsi</p>
                    </div>
                </div>
            ...
            <form action="{{ route('customer.event.cart.store') }}" method="POST">
                ...
                <div class="d-flex d-flex-column sm:d-flex-row align-items-center justify-content-between g-3 pt-6 border-t border border-secondary">
                    <div>
                        <p class="fs-6 text-secondary fw-medium">Total Harga Paket ({{ $package->total_portions }} Porsi)</p>
                        <p class="fs-3 sm:fs-2 fw-bold text-primary mt-0.5">{{ $package->formatted_price }}</p>
                    </div>
