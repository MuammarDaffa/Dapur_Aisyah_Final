...
<div class="row">
    <div class="col-md-8">
        <div class="mb-3">
            @php
                $backServiceId = $paket->layanan_katering_id;
        ...
        <form action="{{ route('admin.paket_katering.update', $paket) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="card card-outline card-primary">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" required value="{{ old('name', $paket->name) }}" class="form-control" placeholder="Nama Paket">
                        @error('name')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                    </div>
                    
                    <div class="mb-3">
                        <label class="form-label fw-bold">Deskripsi</label>
                        <textarea name="deskripsi" rows="3" class="form-control" placeholder="Deskripsi opsional">{{ old('deskripsi', $paket->deskripsi) }}</textarea>
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-bold">Harga (Rp) <span class="text-danger">*</span></label>
                        <input type="text" name="harga" required value="{{ old('harga', number_format($paket->harga, 0, '', '')) }}" class="form-control rupiah-input">
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-bold">Gambar</label>
                        <input type="file" name="image" accept="image/*" class="form-control" id="image-input">
                        <div id="imagePreviewContainer" class="mt-3 {{ $paket->image ? '' : 'd-none' }}">
                            <p class="small text-muted fw-medium mb-1">Preview Gambar:</p>
                            <div class="border rounded bg-light p-1" style="display: inline-block;">
                                <img id="imagePreview" src="{{ $paket->image ? Storage::url($paket->image) : '' }}" alt="Preview Gambar" style="height: 112px; object-fit: cover;">
                            </div>
                        </div>
                    </div>

                    <div class="form-check mb-3">
                        <input class="form-check-input" type="checkbox" name="is_active" value="1" {{ old('is_active', $paket->is_active) ? 'checked' : '' }} id="isActiveCheck">
