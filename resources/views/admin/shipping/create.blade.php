...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Tambah Ongkos Kirim</h2>
            <p class="text-sm text-gray-500 mt-1">Atur biaya pengiriman untuk kecamatan baru</p>
        </div>
    </div>
        ...
        <form action="{{ route('admin.shipping.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" name="cost" id="cost" value="{{ old('cost', 20000) }}" required class="w-full pl-12 pr-4 py-3 rounded-lg border border-gray-200 focus:border-orange-500 focus:ring-2 focus:ring-orange-200 transition-colors bg-gray-50 focus:bg-white rupiah-input">
                </div>
                @error('cost')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="notes" id="notes" rows="3" placeholder="Catatan opsional..."
                          class="w-full border-gray-300 rounded-xl shadow-sm focus:ring-orange-500 focus:border-orange-500 transition-colors">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
