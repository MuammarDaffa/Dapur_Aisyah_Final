...
<div class="max-w-3xl">
    ...
    <form action="{{ route('admin.packages.store') }}" method="POST" enctype="multipart/form-data" class="card shadow-sm mb-4 p-4" id="packageForm">
        @csrf
        ...
        <div class="row row-cols-1 md:row-cols-2 g-3">
            ...
            <label class="form-label fw-bold">Layanan Event *</label>
                @if(request('catering_service_id'))
                    ...
                    @endphp
                    <div class="w-100 px-4 py-2 rounded border bg-light text-secondary fw-medium">
            ...
            <label class="form-label fw-bold">Nama Paket *</label>
                <input type="text" name="name" required value="{{ old('name') }}" class="form-control w-100 px-4 py-2 rounded border focus:border border-primary" placeholder="Contoh: Paket Prasmanan Hemat">
            </div>
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea name="description" rows="2" class="form-control w-100 px-4 py-2 rounded border focus:border border-primary" placeholder="Deskripsi singkat mengenai paket ini...">{{ old('description') }}</textarea>
        </div>

        <div class="row row-cols-1 md:row-cols-2 g-3">
            <div class="mb-3">
            <label class="form-label fw-bold">Harga Paket (Rp) *</label>
                <input type="text" name="price" required value="{{ old('price') }}" class="form-control w-100 px-4 py-2 rounded border focus:border border-primary rupiah-input" placeholder="0">
            </div>
            <div class="mb-3">
            <label class="form-label fw-bold">Total Porsi (Paket Tetap) *</label>
                <input type="number" name="total_portions" required value="{{ old('total_portions', 50) }}" min="1" class="form-control w-100 px-4 py-2 rounded border focus:border border-primary">
        ...
        <div class="border-t pt-6">
            ...
            <div id="loadingOptionsMsg" class="fs-6 text-secondary py-6 text-center border-2 border-dashed rounded bg-light">
