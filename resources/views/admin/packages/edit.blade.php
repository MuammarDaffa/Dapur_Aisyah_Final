...
@section('content')
<div class="max-w-3xl">
    <form action="{{ route('admin.packages.update', $package) }}" method="POST" class="bg-white rounded-xl p-6 shadow-sm border space-y-5" id="packageForm">
        @csrf @method('PUT')
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <select name="catering_service_id" id="serviceSelect" required class="w-full px-4 py-2 rounded-lg border" onchange="loadServiceOptions()">
                    ...
                    <option value="{{ $s->id }}" {{ $package->catering_service_id == $s->id ? 'selected' : '' }}>{{ $s->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block text-sm font-medium mb-1">Nama Paket *</label>
                <input type="text" name="name" required value="{{ old('name', $package->name) }}" class="w-full px-4 py-2 rounded-lg border">
            </div>
        </div>

        <div><label class="block text-sm font-medium mb-1">Deskripsi</label><textarea name="description" rows="2" class="w-full px-4 py-2 rounded-lg border">{{ old('description', $package->description) }}</textarea></div>

        <div class="grid grid-cols-3 gap-4">
            <div><label class="block text-sm font-medium mb-1">Harga Paket (Rp) *</label><input type="text" name="price" required value="{{ old('price', $package->price) }}" class="w-full px-4 py-2 rounded-lg border rupiah-input"></div>
            <div><label class="block text-sm font-medium mb-1">Total Porsi *</label><input type="number" name="total_portions" required value="{{ old('total_portions', $package->total_portions) }}" min="1" class="w-full px-4 py-2 rounded-lg border"></div>
            <div><label class="block text-sm font-medium mb-1">Min. Tambahan</label><input type="number" name="min_addition_qty" value="{{ old('min_addition_qty', $package->min_addition_qty) }}" min="0" class="w-full px-4 py-2 rounded-lg border"><p class="text-xs text-gray-500 mt-1">0 = tanpa minimum</p></div>
        </div>

        {{-- Custom Options Selector --}}
        <div>
            <label class="block text-sm font-bold text-gray-900 mb-4 border-b pb-2">Isi Paket</label>
            <div id="loadingOptionsMsg" class="text-sm text-gray-400 py-4 text-center border-2 border-dashed rounded-lg bg-gray-50 hidden">Memuat opsi...</div>
...
window.existingOptionIds = @json($package->customOptions->pluck('id'));
    ...
    fetch(`/api/service/${serviceId}/custom-options`)
        ...
        .then(data => {
            ...
            if (!window._initialLoadDone && window.existingOptionIds) {
                ...
                currentSelections.menu = window.existingOptionIds;
                currentSelections.extra = window.existingOptionIds;
            } else {
            ...
            data.forEach(opt => {
                const isSelected = window.existingOptionIds ? window.existingOptionIds.includes(opt.id) : false;
