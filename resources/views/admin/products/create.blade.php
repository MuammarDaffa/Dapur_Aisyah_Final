...
<div class="max-w-2xl">
    ...
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl p-6 shadow-sm border space-y-4">
        ...
        <div><label class="block text-sm font-medium mb-1">Extra Tambahan (Opsional)</label><select id="extras-select" name="extras[]" multiple class="w-full px-4 py-2 rounded-lg border" placeholder="Pilih Extra...">@foreach($extras as $extra)<option value="{{ $extra->id }}" {{ in_array($extra->id, old('extras', [])) ? 'selected' : '' }}>{{ $extra->name }}</option>@endforeach</select>@error('extras')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror</div>
        @endif
