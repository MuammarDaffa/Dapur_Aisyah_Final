    ...
    <form action="{{ route('admin.catering.update', $catering) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl p-6 shadow-sm border space-y-5">
        ...
        <div class="grid grid-cols-3 gap-4">
            <div>
                ...
                <input type="number" name="base_price" required value="{{ old('base_price', $catering->base_price) }}"
                       class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:ring-orange-500 focus:border-orange-500">
            </div>
            <div>
        ...
        <div class="bg-orange-50/50 border border-orange-100 rounded-xl p-4 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="minimal_order_days" value="{{ old('minimal_order_days', $catering->minimal_order_days) }}" min="0" placeholder="cth: 3"
                           class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:ring-orange-500 focus:border-orange-500">
                ...
                <div>
                    ...
                    <input type="time" name="cutoff_time" value="{{ old('cutoff_time', $catering->cutoff_time ? substr($catering->cutoff_time, 0, 5) : '') }}"
                           class="w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:ring-orange-500 focus:border-orange-500">
        ...
        <div>
            ...
            @if($catering->image)
            <p class="text-xs text-gray-500 mt-1">📷 Gambar saat ini tersimpan. Upload baru untuk mengganti.</p>
            @endif
        </div>
